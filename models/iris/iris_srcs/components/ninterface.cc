...
#ifdef _DEBUG
                    _DBG(" SEND H/B CREDIT vc%d", ld->vc);
#endif
                    Send(1,ld);
...
NInterface::handle_send_credit_event ( int port, uint64_t data )
{
     ...
     * */
    manifold::kernel::Manifold::Schedule( 1, &IrisTerminal::handle_update_credit_event, 
                                          static_cast<IrisTerminal*>(terminal),(int)SEND_SIG, data);
    return ;
}
...
NInterface::tock ( void )
{
    ...
    if ( !arbiter.is_empty() )
    {
        ...
        if ( f->type == TAIL || f->pkt_length == 1 )
        {
            ...
            //            manifold::kernel::Manifold::Schedule(1, &IrisTerminal::handle_update_credit_event, static_cast<IrisTerminal*>(terminal()), (int)SEND_SIG, winner);
        }
    ...
    // proc_recv flag
    if ( !static_cast<IrisTerminal*>(terminal)->proc_recv )
    {
        ...
        if ( found )
        {
            ...
            static_cast<IrisTerminal*>(terminal)->proc_recv = true;

            //            _DBG(" Int sending pkt to proc %d", last_inpkt_winner); 
            manifold::kernel::Manifold::Schedule(1, &IrisTerminal::handle_new_packet_event, static_cast<IrisTerminal*>(terminal),(uint)SEND_DATA, np);
