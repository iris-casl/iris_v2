...
main ( int argc, char *argv[] )
{
    ...
    while(!fd.eof())
    {
        ...
        if ( simknob.find('-') == 0 )
        {
            ...
            if ( sim_string.compare("-iris") == 0 )
            {
                ...
                if (! key.compare("-iris:dst_distrib"))
                    params.insert(pair<string,string>("dst_distrib",value));
            }
            ...
            else if (! key.compare("-MC_LOC") )
            {
                ...
                std::stringstream tt; tt<<pos;
                std::stringstream tt2; tt2<<"mc_loc:" << mc_positions.size()-1;
                params.insert(pair<string,string>(tt2.str(),tt.str()));
    ...
    if ( Mytid == TERMINAL_PROC )
        cerr << tp->print_stats(IRIS_TERMINAL);
    if ( Mytid == ROUTER_PROC)
        cerr << tp->print_stats(IRIS_ROUTER);
    if ( Mytid == TERMINAL_PROC )
    {
